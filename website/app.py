...
for col in df.iloc[:, 3:]:
    ...
    if df[col].dtype == object and df[col].str.contains("$").any():
        # Remove the percentage sign and convert to float
        df[col] = (
            df[col].str.replace("[\$,]", "", regex=True).astype(float) / 100
        )
        # Now normalize
        df[col] = df[col] / df[col].sum()
# Data Selection
...
def details(county):
    ...
    park_lst = ["Parking Friendliness", [temp_df.columns[12], temp_df.iloc[0, 12]], [temp_df.columns[13], temp_df.iloc[0, 13]]]
    bike_lst = ["Biking Friendliness", [temp_df.columns[2], temp_df.iloc[0, 2]]]
    commute_lst = ["Ease of Commute", [temp_df.columns[15], temp_df.iloc[0, 15]], [temp_df.columns[16], temp_df.iloc[0, 16]]]
    afford_lst = ["Affordability", [temp_df.columns[5], temp_df.iloc[0, 5]], [temp_df.columns[6], temp_df.iloc[0, 6]], [temp_df.columns[7], temp_df.iloc[0, 7]], [temp_df.columns[8], temp_df.iloc[0, 8]]]
    safe_lst = ["Safety", [temp_df.columns[1], temp_df.iloc[0, 1]], [temp_df.columns[3], temp_df.iloc[0, 3]], [temp_df.columns[4], temp_df.iloc[0, 4]], [temp_df.columns[9], temp_df.iloc[0, 9]], [temp_df.columns[10], temp_df.iloc[0, 10]], [temp_df.columns[11], temp_df.iloc[0, 11]]]
